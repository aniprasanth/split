...
class _AddGroupScreenState extends State<AddGroupScreen> {
  ...
  Future<void> _saveGroup() async {
    ...
    try {
      ...
      final success = await dbService.createGroup(group).timeout(
        const Duration(seconds: 30),
        onTimeout: () {
          throw TimeoutException('Group creation timed out. Please try again.');
        },
      );
      ...
      if (success) {
        ...
        // Navigate to group detail screen immediately
      ...
      if (!mounted) return;
      _showErrorSnackBar(e.message ?? 'Group creation timed out');
    } catch (e) {
      if (!mounted) return;
