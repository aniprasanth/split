...
class _GroupDetailScreenState extends State<GroupDetailScreen>
              ...
              ElevatedButton(
                onPressed: _isUpdatingGroup
                  ...
                  try {
                    ...
                    final success = await dbService.updateGroup(updatedGroup).timeout(
                      const Duration(seconds: 30),
                      onTimeout: () {
                        throw TimeoutException('Group update timed out. Please try again.');
                      },
                    );
                    ...
                    if (success) {
                      ...
                      scaffoldMessenger.showSnackBar(
                        SnackBar(
                          content: Text(dbService.errorMessage ?? 'Failed to update group'),
                    ...
                    scaffoldMessenger.showSnackBar(
                      SnackBar(
                        content: Text(e.message ?? 'Group update timed out'),
                    ...
                    ...currentGroup.members.map((member) => ListTile(
                      ...
                      trailing: (member != 'You' && member != currentGroup.createdBy)
                        ...
                        onPressed: _isManagingMembers
                          ...
                          try {
                            // Add timeout to prevent indefinite hanging
                            final success = await dbService.removeMemberFromGroup(currentGroup.id, member).timeout(
                              const Duration(seconds: 30),
                              onTimeout: () {
                                throw TimeoutException('Removing member timed out. Please try again.');
                              },
                            );
                            ...
                            if (success) {
                              ...
                              scaffoldMessenger.showSnackBar(
                                SnackBar(
                                  content: Text('Member "$member" removed successfully!'),
                                  backgroundColor: Colors.green,
                                  behavior: SnackBarBehavior.floating,
                              ...
                              scaffoldMessenger.showSnackBar(
                                SnackBar(
                                  content: Text(dbService.errorMessage ?? 'Failed to remove member'),
                                  backgroundColor: Colors.red,
                                  behavior: SnackBarBehavior.floating,
                                ),
                              );
                            ...
                            scaffoldMessenger.showSnackBar(
                              SnackBar(
                                content: Text(e.message ?? 'Removing member timed out'),
                                backgroundColor: Colors.orange,
                                behavior: SnackBarBehavior.floating,
                              ),
                            );
                            ...
                            if (mounted) setDialogState(() => _isManagingMembers = false);
                          }
                        },
                      )
                          : null,
  ...
  void _addNewMember(TextEditingController controller, StateSetter setDialogState) async {
    ...
    if (currentGroup.members.contains(newMemberId)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Member "$newMemberName" is already in the group'),
          backgroundColor: Colors.orange,
          behavior: SnackBarBehavior.floating,
        ),
      );
    ...
    try {
      // Add timeout to prevent indefinite hanging
      final success = await dbService.addMemberToGroup(currentGroup.id, newMemberId, newMemberName).timeout(
        const Duration(seconds: 30),
        onTimeout: () {
          throw TimeoutException('Adding member timed out. Please try again.');
        },
      );
      ...
      if (success) {
        ...
        scaffoldMessenger.showSnackBar(
          SnackBar(
            content: Text('Member "$newMemberName" added successfully!'),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
            duration: const Duration(seconds: 2),
          ),
        ...
        scaffoldMessenger.showSnackBar(
          SnackBar(
            content: Text(dbService.errorMessage ?? 'Failed to add member'),
            backgroundColor: Colors.red,
            behavior: SnackBarBehavior.floating,
          ),
        );
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: Text(e.message ?? 'Adding member timed out'),
          backgroundColor: Colors.orange,
          behavior: SnackBarBehavior.floating,
        ),
      );
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: _isDeletingGroup
                  ...
                  try {
                    // Add timeout to prevent indefinite hanging
                    final success = await dbService.deleteGroup(currentGroup.id).timeout(
                      const Duration(seconds: 60), // Longer timeout for deletion
                      onTimeout: () {
                        throw TimeoutException('Group deletion timed out. Please try again.');
                      },
                    );
                    ...
                    if (success) {
                      ...
                      scaffoldMessenger.showSnackBar(
                        SnackBar(
                          content: Text(dbService.errorMessage ?? 'Failed to delete group'),
                          backgroundColor: Colors.red,
                          behavior: SnackBarBehavior.floating,
                        ),
                      );
                    ...
                    scaffoldMessenger.showSnackBar(
                      SnackBar(
                        content: Text(e.message ?? 'Group deletion timed out'),
                        backgroundColor: Colors.orange,
                        behavior: SnackBarBehavior.floating,
                      ),
                    );
